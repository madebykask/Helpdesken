...
namespace DH.Helpdesk.Services.Services
{
 ...
 #region Send email about new case to notifier or tblCustomer.NewCaseEmailList & (productarea template, priority template)
        ...
        public bool RemoveDataPrivacyFromCase(int customerId, List<string> fieldsNames, DateTime? replaceDatesWith, 
            string replaceDataWith, DateTime? registerDateFrom, DateTime? registerDateTo, bool closedOnly, 
            bool removeCaseAttachments, bool removeLogAttachments, bool removeCaseHistory)
        {
            using (var uow = unitOfWorkFactory.Create())
            {
                ...
                var cases = rep.GetAll().Where(x => x.Customer_Id == customerId);

                if (closedOnly)
                    cases = cases.Where(x => x.FinishingDate.HasValue);
                if (registerDateFrom.HasValue)
                    cases = cases.Where(x => x.RegTime >= registerDateFrom.Value);
                if (registerDateTo.HasValue)
                    cases = cases.Where(x => x.RegTime <= registerDateTo.Value);
                ProcessReplaceCasesData(cases, caseFiles, logFiles, caseHistories, emailLogs, fieldsNames, replaceDataWith, replaceDatesWith, removeCaseAttachments, removeLogAttachments, removeCaseHistory);
                uow.Save();
                return true;
            }
        }
        ...
        private void ProcessReplaceCasesData(IQueryable<Case> cases, 
            Dal.NewInfrastructure.IRepository<CaseFile> caseFiles, 
            Dal.NewInfrastructure.IRepository<LogFile> logFiles, 
            Dal.NewInfrastructure.IRepository<CaseHistory> caseHistories, 
            Dal.NewInfrastructure.IRepository<EmailLog> emailLogs, 
            List<string> fieldsNames, string replaceDataWith, DateTime? replaceDatesWith, bool removeCaseAttachments, 
            bool removeLogAttachments, bool removeCaseHistory)
        {
            foreach (var c in cases)
            {
                foreach (var cfn in fieldsNames)
                {
                    switch (cfn)
                    {
                        #region initiator

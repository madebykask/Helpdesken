                ...
                if(SessionFacade.TextTranslation != null)
                {
                    try
                    {
                        ...
                        if(translation != null)
                        {
                            ...
                            var trans = translation.TextTranslations.Where(x => x.Language_Id == ((languageId > 0) ? languageId : SessionFacade.CurrentLanguageId)).FirstOrDefault();

                            if(trans != null && !string.IsNullOrEmpty(trans.TextTranslated))
                                translate = trans.TextTranslated;
                        }
                    }
                    catch
                    {

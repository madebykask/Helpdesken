...
namespace ECT.Web.Areas.Global.Controllers
{
    public class HiringController : BaseController
    {
        ...
        public ActionResult Contract(string id, int caseId, string path)
        {
            ...
            var page = GetNewHirePdfPage(model.CustomerId,model.Contract.Company.Key, lang);
            var workStream = new MemoryStream();
            
            var document = new Document(PageSize.A4, 42, 45, 100, 70);        
        ...
        private NetherlandsPdfPage GetNewHirePdfPage(int customerId, string searchKey, string language)
        {
            var pdfPage = new NetherlandsPdfPage();
            var department = _contractRepository.GetDepartmentById(customerId, searchKey);

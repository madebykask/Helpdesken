...
namespace DH.Helpdesk.EForm.FormLib.Areas.SouthKorea.Controllers
{
    ...
    {
        public const string xmlPath = "southkorea/hiring.xml";
        ...
        //public JsonResult BusinessUnitData(string query)
        //{
        ...
        public override ActionResult New(FormCollection formCollection, string[] uploads)
        {
            ...
            if (actionStateChange)
            {
                if (!string.IsNullOrEmpty(actionState) && model.IsValid(ref dictionary, int.Parse(actionState)))
                {
                    ...
                    {
                        model.ReplaceNode("state", "southkorea/templates/hiringbasiconestate.xml");
                    }
        ...
        public ActionResult Contract(string id, int caseId, string path)
        {
            ...
            //43 mm in points 121.88976378f
            var document = new Document(PageSize.A4, iTextSharp.text.Utilities.MillimetersToPoints(15), 43, iTextSharp.text.Utilities.MillimetersToPoints(33), iTextSharp.text.Utilities.MillimetersToPoints(30));
            ...
            var html = RenderRazorViewToString("~/FormLibContent/Areas/SouthKorea/Views/Hiring/Pdfs/" + id + ".cshtml", model, false);
            ...
            using (TextReader htmlViewReader = new StringReader(html))
            {
                ...
                using (var htmlWorker = new HTMLWorkerExtended(document))
                {
                    ...
                    if (lang == "en")
                    {
                        textFont = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Fonts), "verdana.TTF");
                        iTextSharp.text.FontFactory.Register(textFont, "verdana");
                        fontName = "Verdana";
                    }

                    else
                    {
                        textFont = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Fonts), "Malgun.TTF");
                        iTextSharp.text.FontFactory.Register(textFont, "Malgun");
                        fontName = "Malgun";

                    }
                    htmlWorker.Open();
                    var styleSheet = new iTextSharp.text.html.simpleparser.StyleSheet();
                    styleSheet.LoadTagStyle(HtmlTags.BODY, HtmlTags.FACE, fontName + " Unicode MS");
        ...
        private SouthKoreaPdfPage GetPdfPage(int customerId, string searchKey, string language, string id, Department department, FormModel model)
        {

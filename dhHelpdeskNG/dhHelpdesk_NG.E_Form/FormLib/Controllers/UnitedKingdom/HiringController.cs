...
namespace DH.Helpdesk.EForm.FormLib.Areas.UnitedKingdom.Controllers
{
    ...
    {
        public const string xmlPath = "unitedkingdom/hiring.xml";
        ...
        public override ActionResult New(FormCollection formCollection, string[] uploads)
        {
            ...
            if(actionStateChange)
            {
                if(!string.IsNullOrEmpty(actionState) && model.IsValid(ref dictionary, int.Parse(actionState)))
                {
                    ...
                    {
                        model.ReplaceNode("state", "unitedkingdom/templates/hiringbasiconestate.xml");
                    }
        ...
        public ActionResult Contract(string id, int caseId, string path)
        {
            ...
            var html = RenderRazorViewToString("~/FormLibContent/Areas/UnitedKingdom/Views/Hiring/Pdfs/" + id + ".cshtml", model, false);
            ...
            using(TextReader htmlViewReader = new StringReader(html))
            {
                ...
                using(var htmlWorker = new HTMLWorkerExtended(document))
                {
                    ...
                    styleSheet.LoadTagStyle(HtmlTags.UL, HtmlTags.INDENT, "20");
                    styleSheet.LoadTagStyle(HtmlTags.OL, HtmlTags.INDENT, "10");
                    styleSheet.LoadTagStyle(HtmlTags.LI, HtmlTags.INDENT, "15");
        ...
        private UnitedKingdomPdfPage GetPdfPage(int customerId, string searchKey, string language, string id, Department department, FormModel model)
        {
            var pdfPage = new UnitedKingdomPdfPage();

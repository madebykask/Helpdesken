...
ExtendedCasePage.prototype.setCaseStatus = function (status) {
    ...
    switch (status) {
        case self.CASE_IN_IDLE:
            case_Is_In_Saving_Mode = false;
            $caseButtonsToLock.removeClass('disabled');
            $caseButtonsToLock.css("pointer-events", "");
            return true;

        case self.CASE_IN_SAVING:
            case_Is_In_Saving_Mode = true;
            $caseButtonsToLock.addClass('disabled');
            $caseButtonsToLock.css("pointer-events", "none");
            return true;

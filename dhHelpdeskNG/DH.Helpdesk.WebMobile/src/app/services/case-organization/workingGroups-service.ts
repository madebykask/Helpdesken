...
export class WorkingGroupsService extends HttpApiServiceBase {
    ...
    getWorkingGroups(customerId?: number) {
      const params = isNaN(customerId) ? {} : {cid: customerId};
        return this.getJson(this.buildResourseUrl('/api/workinggroups/options', params, isNaN(params.cid)))
        .pipe(
            take(1),
            map((jsItems: any) => {
    ...
    getWorkingGroup(id: number, customerId: number) {
      return this.getJson(this.buildResourseUrl(`/api/workinggroups/${id}`, { cid: customerId }, false, true))
      .pipe(
          take(1),
          map((jsItems: any) => {

			...
			if (!this.IsInline)
			{
				...
				if (request.Browser.Browser.ToUpper() == "IE" ||
					request.UserAgent.IndexOf("Edge", StringComparison.OrdinalIgnoreCase) > -1
						|| (request.UserAgent.IndexOf("MSIE", StringComparison.OrdinalIgnoreCase) >= 0)
						|| (request.UserAgent.IndexOf("Trident", StringComparison.OrdinalIgnoreCase) >= 0)
						|| (request.UserAgent.IndexOf("rv:11.0", StringComparison.OrdinalIgnoreCase) >= 0))
				{

...
namespace DH.Helpdesk.Web.Controllers
{
    ...
    public sealed class FaqController : BaseController
    {
        ...
        {
            var faqOverviews = _faqService.FindOverviewsByCategoryId(categoryId, SessionFacade.CurrentLanguageId);

            var faqModels =
                faqOverviews.Select(f => new FaqOverviewModel(f.Id, f.CreatedDate, f.ChangedDate, f.Text)).ToList();

            return Json(faqModels, JsonRequestBehavior.AllowGet);
        }
        
        [HttpGet]
        public JsonResult FaqsDetailed(int categoryId)
        {
            var faqDetailedOverviews = _faqService.FindDetailedOverviewsByCategoryId(categoryId, SessionFacade.CurrentLanguageId);
            var faqIds = faqDetailedOverviews.Select(f => f.Id).ToList();
            var faqFiles = _faqFileRepository.FindFileOverviewsByFaqIds(faqIds);
            ...
            foreach (var faqDetailedOverview in faqDetailedOverviews)
            {
                var faqModel = new FaqDetailedOverviewModel(
                    ...
                    faqDetailedOverview.ChangedDate,
        ...
        public ViewResult Index(bool showDetails = false)
        {
            ...
            {
                model = _indexModelFactory.Create(null, null, null, userHasFaqAdminPermission);
                return View(model);
            }
            ...
            var faqs = _faqService.FindOverviewsByCategoryId(firstCategoryId, SessionFacade.CurrentLanguageId);
            model = _indexModelFactory.Create(categoriesWithSubcategories, firstCategoryId, faqs, userHasFaqAdminPermission);            
        ...
        {
            var faqOverviews = _faqService.SearchOverviewsByPharse(pharse, SessionFacade.CurrentCustomer.Id, SessionFacade.CurrentLanguageId);

            var faqModels =
                faqOverviews.Select(
                    f => new FaqOverviewModel(f.Id, f.CreatedDate, f.ChangedDate, f.Text)).ToList();
        ...
        {
            var faqDetailedOverviews = _faqService.SearchDetailedOverviewsByPharse(

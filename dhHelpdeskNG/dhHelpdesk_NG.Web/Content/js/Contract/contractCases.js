function ContractCases(pageSettings) {
    'use strict';
    ...
    (function ($) {
        ...
        CTP.prototype.init = function (appSettings) {
            ...
            self.$freeTextSearch = $('#txtFreeTextSearch');
            self.caseNumberSearchPrefix = appSettings.caseNumberSearchPrefix;

            self.setCaseNumbers(appSettings.caseNumbers);
                ...
                appSettings.perShowingText,
                {
                    ...
                    createdRow: function (row, data, dataIndex) {
                        if (data) {
                            ...
                            row.cells[0].innerHTML = '<a href="/Cases/Edit/' +
                                data.case_id +
                                '"><img title="' +
                                data.caseIconTitle +
                                '" alt="' +
            ...
            self.table.on('xhr.dt',
                    function (e, settings, json) {
                        ...
                        if (json && json.result === 'success') {
                            ...
                            var newCaseType = parseInt($('#lstfilterCaseProgress').val(), 10);
        ...
        CTP.prototype.getFetchParams = function () {
            ...
            var baseParams = $('#frmCaseSearch').serializeArray();;
        ...
        */
        ...
        CTP.prototype.formatCell = function (caseId, cellValue) {
            var out = ['<a href="/Cases/Edit/' + caseId + '">' + (cellValue == null ? '&nbsp;' : cellValue.replace(/<[^>]+>/ig, '')) + '</a>'];
            return out.join('');
            //return [cellValue.replace(/<[^>]+>/ig, '')];
        };

        CTP.prototype.setCaseNumbers = function (caseNumbers) {
            var self = this;
            if (caseNumbers) {
                self.$freeTextSearch.val(self.caseNumberSearchPrefix + caseNumbers);
            }
        }

        CTP.prototype.reload = function () {
            var self = this;
            self.table.ajax.reload.call(self);
        }

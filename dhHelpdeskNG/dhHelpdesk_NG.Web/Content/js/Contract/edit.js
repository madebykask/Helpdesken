$(function () {    
    ...
    (function ($) {
        ...
        window.Application.contract = window.Application.contract || {};


        var uploadContractFileUrl = window.appParameters.uploadContractFileUrl;
        var ContractFileKey = window.appParameters.ContractFileKey;
        var contractId = window.appParameters.contractId;
        var ContractFilesUrl = window.appParameters.ContractFilesUrl;
        ...
        Application.contract.init = function () {
            //customerId = $('#NewContract_Customer_Id').val();           
        };
             
        var lastInitiatorSearchKey = ''
        var _pluploadContract;
        ...
        Application.contract.bindDeleteNewFiles = function () {
            $('#newFiles a[id^="newFileDeleteButton"]').click(function () {
                Application.contract.deleteFile($(this));
            });
        };

        Application.contract.deleteFile = function ($elm) {
            var $self = $elm;
            var fileId = $self.attr('data-id');
            var fileName = $self.attr('data-fileName');
            var elementToDelete = $self[0].parentNode.parentNode;
            var fPlace = (fileId == 0) ? ContractFileKey : contractId;

            $.get(deleteContractFileUrl, { id: fileId, fileName: fileName, filePlace: fPlace, myTime: Date.now() }, function (res) {
                if (res != null) {
                    if (res.result)
        ...
        Application.contract.initFiles = function () {
            if (contractId > 0) {
                $.get(ContractSavedFilesUrl, { id: ContractFileKey, myTime: Date.now(), cId: contractId }, function (files) {

                    Application.contract.refreshFilesTable(files);
                });
            }
        };

        Application.contract.reloadFiles = function () {
            $.get(ContractFilesUrl, { id: ContractFileKey, myTime: Date.now(), cId: contractId }, function (files) {
                Application.contract.refreshFilesTable(files);
            });
        };
            ...
            init: {
                ...
                StateChanged: function (uploader) {
                    ...
                    for (var i = 0; i < alreadyExistFileIds.length; i++) {
                        ...
                        $('#Contract_uploader ul[class="plupload_filelist"] li[id="' + fileId + '"] div[class="plupload_file_action"] a').prop('title', fileAlreadyExistsMsg);
                    }
        ...
        $('#Contract_upload_files_popup').on('hidden.bs.modal', function () {
            if ($('#Contract_uploader') != undefined) {
                if ($('#Contract_uploader').pluploadQueue().files.length > 0) {
                    if ($('#Contract_uploader').pluploadQueue().state == plupload.UPLOADING)
                        $('#Contract_uploader').pluploadQueue().stop();

                    while ($('#Contract_uploader').pluploadQueue().files.length > 0) {
                        $('#Contract_uploader').pluploadQueue().removeFile($('#Contract_uploader').pluploadQueue().files[0]);
                    }
                    $('#Contract_uploader').pluploadQueue().refresh();
                }
            }
        });

        Application.contract.refreshFilesTable = function (files) {
            $('#newFiles > tr').remove();
            var fileMarkup;
            for (var i = 0; i < files.length; i++) {
                ...
                $('#newFiles').append(fileMarkup);
            }

            Application.contract.bindDeleteNewFiles();
        }
        
        Application.contract.generateRandomKey = function () {
            function s4() {
            ...
            this.allowSave = function (blob) {                
                ...
                if (source == 'contract') {
                    ...
                    refredhCallback = function () {
                        $.get(ContractFilesUrl, { id: ContractFileKey, curTime: Date.now(), cId: contractId }, function (files) {
                            Application.contract.refreshFilesTable(files);
                        });
                    }
                }
                ...
                imgFilename = 'image_' + Application.contract.generateRandomKey();
        ...
        function clearScene() {
            curFileName = 'image_' + Application.contract.generateRandomKey();
            $("#previewPnl").empty();
            var $uploadModal = $('#upload_clipboard_file_popup');
            var $btnSave = $uploadModal.find('#btnSave');
            $btnSave.hide();
